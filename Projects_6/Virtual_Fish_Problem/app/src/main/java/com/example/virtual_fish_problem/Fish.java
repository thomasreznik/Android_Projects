...
public class Fish {
    ...
    public Fish(int xPos, int yPos, int condition, int tankWidth, int tankHeight){
        ...
        mTankHeight = tankWidth;
        x = xPos;
        y = yPos;
        mDirection = 1;
        //FOOD AND EXPLORE LOCATIONS ARE FIXED
        // IN THE TOP AND BOTTOM OF THE TANK
        foodX = (int) tankHeight / 2 - 100;
        foodY = (int) (Math.ceil(Math.random() * mTankWidth) - mTankWidth / 2);
        playX = (int) -(Math.random() * mTankHeight / 2) + 100;
        playY = (int) (Math.ceil(Math.random() * mTankWidth) - mTankWidth /2);
    }
    public void move() {
    ...
    private void swim() {
        ...
        //TASK 3: FID ANOTHER PLACE TO EXPLORE IN THE TOP HALF OF THE TANK
        if(Math.abs(xDistance) < 5 && Math.abs(yDistance) < 5){
            playX = (int) (Math.ceil(Math.random() * mTankHeight) - mTankWidth / 2);
